...
int main(int argc, char *argv[])
{
	...
	if (device_name) {
		...
		for (size_t j = 0; j < 1; j++)
		{
			...
			for (i = 0; devs[i]; i++)
			{
				if (j == 0 && devs[i]->device_descriptor.idVendor == 0x1B1C && devs[i]->device_descriptor.idProduct == 0x1BFE)
				{
					stored_device[index++] = devs[i];
					printf("keep hexagon device\n");
					libusb_keep_device(devs[i]);
				}
			}
	...
	for (size_t t = 0 ; t<index ; t++)
	{
		...
		printf("list ref devices\n");
		libusb_listref_devices(NULL);
		libusb_unkeep_device(stored_device[t]); /* should clear all devices list */
	}

	printf("list ref devices (should be empty)\n");
	libusb_listref_devices(NULL);
	libusb_exit(NULL);
	return r;
}
